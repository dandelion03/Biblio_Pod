...
export const HomePage = () => {
    ...
    if (files.length > 0) {
      try {
        ...
        for (const file of files) {
          ...
          const additionalDetails = await fetchBookDetails(bookInfo.title, bookInfo.author, bookInfo.language);
        ...
        setFileDetails((prevFileDetails) => [...prevFileDetails, ...updatedFileDetails]);
  ...
  const fetchBookDetails = async (title,author,language) => {
    const urlApi= `https://www.googleapis.com/books/v1/volumes?q=${title+" "+author+" "+"&langRestrict=en"}`
    console.log(urlApi)
    try {
  ...
  return (
    ...
    <NewNav/>
      ...
      <Input id="picture" className="cursor-pointer" onChange={handleFileChange} placeholder="add more books" multiple accept=".epub" type="file" /></div>
 </div>
...
<div className='main-page-coll-pt-inner flex justify-between items-center '>
        ...
        <div><Link to="/Collections"><MdNavigateNext className="cursor-pointer" /></Link>
        </div>
    ...
    <div key={index}>
      ...
      />
          ...
          <div className="empty-Lib-container">
            ...
            <div>add as many books as you want by dropping the epub files or select them from your local storage</div>
            <div className="input-centered ">  
      <Input id="picture" className="cursor-pointer" multiple onChange={handleFileChange} accept=".epub" type="file" /></div>
            </div>}
          </div>
